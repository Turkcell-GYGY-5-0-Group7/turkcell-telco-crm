...
| `addon.purchased.v1` | order-service | usage, billing | One event per ADDON order item when the owning order is fulfilled (Sprint 24 feature 24.3, design-note D1/D3). Outbox aggregate_type `addon`, aggregate_id = order-item id, topic `addon.events`. usage-service tops up the target subscription's quota with the snapshotted allowance deltas; billing-service records an `addon_charge_record` billed as one invoice line on the next bill run. |
